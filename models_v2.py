...
from rotation_conversions import quaternion_apply, axis_angle_to_quaternion
base = 100
import os
import math
...
def precompute_freqs_cis(seq_len, dim, device):
    ...
    t = torch.arange(1, seq_len+1, device=device)
    freqs = torch.outer(t, freqs) #(seq_len, dim)
    return freqs


def apply_geo_emb(x: torch.Tensor, freqs_cis: torch.Tensor, flag) -> torch.Tensor:
    """
    Applies geometric positional embeddings to the input tensor.
    ...
    Returns:
        torch.Tensor: Tensor with geometric embeddings applied.
    """
    dtype = x.dtype
    ...
    if flag:
        q = q.unsqueeze(2) - q.unsqueeze(1) #(b, n, n, d, q)
        x = x.unsqueeze(1)
        x_rot = quaternion_apply(q_rot, x)
        q_rot = axis_angle_to_quaternion(q)
        y = rearrange(x_rot, 'b m n d q -> b m n (d q)')
    else:
...
class Attention(nn.Module):
    ...
    def forward(self, x, flag):
        ...
        freqs_cis = precompute_freqs_cis(int(math.sqrt(n)), q.shape[-1], q.device)
        q, k = apply_geo_emb(q, freqs_cis), apply_geo_emb(k, freqs_cis)
        
        if flag:
            k = apply_geo_emb(k, freqs_cis, flag)
            q = q * self.scale
            attn = torch.einsum('bnd, bmnd->bmn', q, k)
        else:
            q, k = apply_geo_emb(q, freqs_cis), apply_geo_emb(k, freqs_cis)
...
class Layer_scale_init_Block(nn.Module):
    ...
    def forward(self, x, flag):
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), flag))
...
class vit_models(nn.Module):
    ...
    def forward_features(self, x, flag=False):
        B = x.shape[0]
...
def deit_tiny_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,   **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=198, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
...
def deit_medium_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False, **kwargs):
    model = vit_models(
        patch_size=16, embed_dim=528, depth=12, num_heads=8, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Layer_scale_init_Block, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
def deit_base_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(#792
        img_size = img_size, patch_size=16, embed_dim=792, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
    if pretrained:
...
def deit_large_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=1056, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
    if pretrained:
...
def deit_huge_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=14, embed_dim=1296, depth=32, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Layer_scale_init_Block, **kwargs)
    if pretrained:
...
def deit_huge_patch14_52_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=14, embed_dim=1296, depth=52, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Layer_scale_init_Block, **kwargs)
...
def deit_huge_patch14_26x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=14, embed_dim=1296, depth=26, num_heads=16, mlp_ratio=4, qkv_bias=True,
...
def deit_Giant_48x2_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=14, embed_dim=1680, depth=48, num_heads=16, mlp_ratio=4, qkv_bias=True,
...
def deit_giant_40x2_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=14, embed_dim=1440, depth=40, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Block_paral_LS, **kwargs)
    return model
...
def deit_small_patch16_36_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=396, depth=36, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
...
def deit_small_patch16_18x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=396, depth=18, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block_paralx2, **kwargs)
...
def deit_base_patch16_18x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=792, depth=18, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block_paralx2, **kwargs)
...
def deit_base_patch16_36x1_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=792, depth=36, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
